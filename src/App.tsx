...
function VoiceSettings({ pack, error }: { pack: VoicePack | null; error: string }) {
  ...
  return <div className="settings-card voice-settings"><h2>Open-source Gulf voice</h2>
    <p>Lahja uses one fixed synthetic Saudi/Khaleeji speaker generated by an Apache-2.0 model. An online Arabic voice is used temporarily for a newly added phrase that is not in the pack yet.</p>
...
function Settings({ settings, setSettings, voicePack, voiceError, sync, syncState, syncMessage, lastSync, onProgressImported, onReset }: {settings:AppSettings; setSettings:(s:AppSettings)=>void; voicePack:VoicePack|null; voiceError:string; sync:()=>void; syncState:string; syncMessage:string; lastSync:string|null; onProgressImported:()=>void; onReset:()=>void}) {
  ...
  return <section><div className="page-title"><span>Settings</span><h1>Data, sync and installation</h1></div>
    ...
    <div className="settings-card"><div className="settings-heading"><h2>Data source</h2><details className="sheet-help"><summary aria-label="How to create your own phrase list">i</summary><div><strong>Create your own list</strong><ol><li>Make a copy of the default Google Sheet.</li><li>Keep a tab named <b>Vocabulary Mastery</b> with the columns <b>Arabic</b>, <b>English Pronunciation</b>, and <b>Lesson #</b>.</li><li>In Google Sheets, choose <b>Share</b> and set General access to <b>Anyone with the link — Viewer</b>.</li><li>Paste the copied sheet's URL below and press <b>Sync now</b>.</li></ol></div></details></div><label>Adapter<select value={settings.adapter} onChange={e=>save({...settings,adapter:e.target.value as AppSettings['adapter']})}><option value="apps-script">Public Google Sheet or Apps Script</option><option value="csv">Published CSV</option><option value="fallback">Bundled fallback only</option></select></label>
